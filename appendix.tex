...
\begin{summary}
  In diesem Kapitel simuliere ich drei Verfahren, um optische Schnitte
  aus strukturiert beleuchteten Rohbildern zu errechnen. Dafuer nutze
  ich das wellenoptische Modell der Bildformung um aus einer
  analytisch beschriebenen dreidimensionalen Fluorophorverteilung mit
  Linie, Rechtecken und einem dreidimensionalen Objekt Bilder mit
  strukturierter Beleuchtung zu ermitteln.
...
Berechne ein Objekt bestehend aus eine Linie, zwei Rechtecken in
unterschiedlichen Z-Positionen und einer grossen 3D Kugelschale.
\begin{lstlisting}[style=mymatlab]
center_ref = @(a) a(floor(size(a,1)/2),floor(size(a,2)/2),floor(size(a,3)/2));

GX = 64; g2 = newim(GX,GX,GX); % make empty 3d image
