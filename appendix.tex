...
\end{lstlisting}
With the code above, I load all raw images into Matlab, even some data
where the spots were not illuminated. I manually account for this in
the following code.
...
the result by gray value and only use the largest.  The measured 100
camera coordinate pairs in \verb!posmax! correspond to $\r^c_i$ in
equation \ref{eq:rigid-sum}.
 
From Matlab I create the file \verb!fit.max! with batch commands for
Maxima. Then I run Maxima. When it is finished, after a few seconds,
the file \verb!max.out! will contain the four fitted parameters.
\begin{lstlisting}[style=mymatlab]
    c = double(posmax)'; cmd =
