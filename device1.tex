...
The fluorescent plane is directly on the underside of the coverslip,
ensuring good image quality and no (or negligibly small) distortion.
...
rotation matrix (or a rotation matrix with a reflection if
$q=-1$). The value of $q$ can be either $+1$ or $-1$. The value of $q$
is $-1$, if there is a single axis reflection. It is $+1$ if there is
no reflection.
\begin{figure}[!hbt]
  \centering
...
the microscope body and can be easily rotated. So when the camera was
moved, ideally, only the rotation angle $\phi$ needs to be determined
again. Similarly, the focal length setting of the illumination
tubelens $\textrm{TL}_\textrm{ill}$ mainly affects the scale $s$.
\subsubsection{Computational parameter estimation}
Having measured a set of $n\ge 4$ tuples $(\r^c_i,\r^d_i)$ with camera
...
In appendix \ref{seq:app_map} I show the code for the free computer
algebra system Maxima \citep{Maxima.sourceforge.net2013}. With Maxima
the code is particularly simple because it transparently calculates
the necessary derivatives. In addition Maxima can easily be integrated
in my real-time system because it is written in Common Lisp as well.
