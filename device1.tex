...
Um die Geschwindigkeit der Bildaufnahme in unserem Beleuchtungssystem
zu erhoehen, lege ich Wert darauf, mit moeglichst wenigen strukturiert
beleuchteten Rohbildern pro Slice auszukommen. Besonders vorteilhaft
hat sich dabei eine Beleuchtung mit einem sinusfoermigen Gitter
erwiesen. Bei bekannter Gitterperiode lassen sich viele der
Bildverarbeitungsprobleme elegant im Fourierraum ausdruecken. Mit der
HiLo Methode genuegen bereits zwei Aufnahmen mit Gitterbeleuchtung,
zwischen denen das Gitter um eine halbe Gitterperiode verschoben wird,
um eine optische section zu zu Berechnen.
...
daher als Maximum der $N$ strukturiert beleuchteten Bilder $I_n$
zugaenglich ist, wobei $n$ die fuer die Phase des Gitters steht. Der
out-of-focus Hintergrund bleibt in allen Rohbilder gleich und kann
demnach mit dem Minimum der Phasenbilder approximiert werden.
\begin{align} \label{eq:Iminmax}
  I_\textrm{min-max}(x,y) = \max_{n=1\ldots N}(I_n(x,y))- \min_{n=1\ldots N}(I_n(x,y))
\end{align}
Diese Methode hat jedoch den Nachteil, dass eine Vergroesserung der
Anzahl ausgelesener Rohbilder $N$, nicht das Rauschen im
rekonstruierten Bild mindert.  Die Methode der homodynen Demodulation
\citep{Neil1997} erlaubt die Trennung von modulierten und
unmodulierten Signal wenn mindestens 3 Rohbilder verfuegbar sind:
\begin{align} \label{eq:Ihomodyne}
  I_\textrm{homodyne}(x,y) = \left| \sum_{n=1\ldots N} I_n(x,y)\ e^{2\pi i n/N}\right|
\end{align}
Insbesondere tragen bei diesem Ansatz die Messdaten aller Rohbilder
zum Signal der bei und mindern das Photonenrauschen in der optischen
Section.

In \figref{fig:hilo-sec-comparison} habe ich diese
...
habe ich die in \figref{fig:hilo-sec-comparison} dargestellten Im
...
separieren. Bei der einfachen 'min-max' entstehen jedoch
Streifenartefakte. Diese wuerden erst verschwinden, wenn deutlich mehr
Rohbilder aufgenommen wuerden. Homodyne sieht sehr gut aus braucht
aber mindestens 3 Bilder. Die HiLo Methode kommt im Gegensatz mit nur
zwei Aufnahmen aus und ich erklaere den Algorithmus folgenden.  Die
HiLo Methode wurde urpruenglich fuer Speckle illumination entwickelt
\citep{Ventalon2005,Ventalon2007,Lim2011} kann jedoch auch fuer
Gitterbeleuchtung eingesetzt werden \citep{Bozinovic2008a,Mertz2010}.


Angenommen das Gitter variiert in die x-Richtung.  Die Information in
den zwei Rohbildern kann aufgeteilt werden in \citep{Mertz2010} die
...
otf breite kz-Ausdehnung aufweise (siehe \figref{fig:missing-cone}).
...
abgeschnitten. Bei der praktischen Umsetzung bedeutet das, dass die
Gitterfrequenz im Bild bekannt sein muss und beispielsweise an einer
fluoreszenten Ebene gemessen werden sollte.
\begin{figure}[htbp]
  \centering
...
In der Simulation wie auch im Prototypen werden feine Gitter durch 3
Ordnungen abgebildet. Dabei werden die 1st orders durch die OTF
abgeschwaecht, verglichen mit der nullten Ordnung (siehe
\figref{fig:hilo-sec-Illum2}~right).  Beleuchtungskontrast ist hier
verhaeltnismaessig gering aber simulation waere schwieriger
geworden. Im realen System kann mit pupil plane SLM der kontrast
optimiert werden. Am besten waere jedoch, wenn der focal plane SLM als
Phasen SLM genutzt werden wuerde um Zweistrahlinterferenz im Sample zu
erzeugen. Dies ist mit dem aktuellen Protypen jedoch nicht realisiert.

Mit dem Code im Appendix koennten die Methoden hinsichtlich ihres
Verhaltens bei verrauschten Signalen analysiert werden. Der Code
existiert zwar schon, ich habe jedoch im Rahmen dieser Arbeit keinen
Vergleich gemacht.

Ortsfrequenz fuer bestes Sectioning
mit unserem Prototypen sind wir da nicht voellig frei
...
nonlinear algorithms shot noise biases results
