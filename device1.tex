...
(but I used four to obtain \figref{fig:hilo-sec-comparison}~d).
...
Assuming the grating varies in the $x-$direction, the information in
the two raw images can then be expressed as a superposition of
modulated in-focus information $I_\textrm{in}$ and out-of-focus
information $I_\textrm{out}$ that is the same in all raw images
\citep{Mertz2010}:
\begin{align}
...
%expand(i0+i1);
%expand(i0-i1)
Where the sum of the raw images corresponds to a widefield image with
...
quite small, so that other order of the spatial frequency spectrum of
...
re-weight the anisotropic sample information surrounding the first
order in $I_\textrm{demod}$.  Both $\eta$ and $k_g$ must be determined
for each experiment.  The algorithm in \citet{Mertz2010} does not
require to specify $k_g$ but their algorithm is non-linear and
therefore gives biased results in the presence of photon shot noise.
\begin{figure}[htbp]
  \centering
...
results. In the examples of this section a grating pattern of high
spatial frequency (with the first order at $\nu_x\approx 1.3
\textrm{NA}/\lambda_0$ (see orange line in
\figref{fig:hilo-sec-Illum2}~right).  The best $z-$resolution would be
expected for $\nu_x= 1 \textrm{NA}/\lambda_0$, since there the support
along $z$ of the three-dimensional optical transfer function is widest
(see \figref{fig:missing-cone}), with smaller spatial frequencies the
modulation contrast $m$ increases but so does the overlap with
adjacent orders.  The source code in appendix \ref{sec:sectioning}
should be a good starting point to investigate these questions.
