...
The smallest distance between the orders in \figref{fig:mma-fft}~right  \label{sec:etendue-mma}
...
hundred frames of image data at the beginning of an experiment and
keep them in local storage. Images can then be selected by relatively
...
\cma{camera is master} slowest device is chosen as the master. It
...
This allows an Arduino microcontroller to initiated a time-delayed
...
The USB LCoS controller for the focal plane SLM can display its images
only for certain discrete times (\unit[20]{ms}, \unit[10]{ms},
\unit[5]{ms}, \unit[200]{$\mu$s}).  This is because it needs to
programmed with sequences which are supplied by the manufacturer and
it is not straight forward to change them via USB interface. Therefore
we always work with a fixed LCoS display time of \unit[20]{ms}. 
...
variable integration time. In particular a Texas Instruments DMD would
support variable integration times \citep{Guide2012}.


The camera acquisitions are stopped by the control software, whenever
the XYZ-stage is moved.

\begin{figure}[H]
 \centering
 \svginput{1}{memi-electronics}
 \caption{The camera signals the Arduino microcontroller \unit[1]{ms}
   before its integration starts. The microcontroller then starts the
...
Eine Grundvorraussetzung dafuer, dass die mit dem Raytracer
ermittelten SLM Muster das Sample wie geplant bestrahlen ist, dass die
Abbildung (mapping) zwischen Pixeln des focal plane SLM und der Kamera
bekannt ist. Um diese zu messen, verwende ich als Sample eine
fluoreszente Ebene, schalte einzelne Pixel des focal plane SLM ein und 
nehme Kamerabilder auf.

Die fluoreszente Ebene befindet sich direkt auf der Unterseite des
Coverslips. Die Abbildung ist daher nahezu ideal mit keiner (oder
vernachlaessigbar geringer) Distortion.

\subsection{The rigid coordinate transform}

- da eine ebene abgebildet wird kann kann minimal vier freiheitsgrade

- prinzipbedingt

- rotation veraendert sich am meisten

- skalierung bleibt gleich, und optische achse bleibt gleich

- das vereinfacht es ungemein verglichen mit einer affinen
transformation mit 4 unabhaengigen eintraegen in der matrix
(zusaetzlich scherung und anisotrope skalierung) also insgesamt 6
freien parametern
...
is a translation vector and $R_\phi$ is a rotation matrix (or a
