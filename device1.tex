...
$p\in [-N/2,N/2+1]$.  Der Einfachheit halber vernachlaessige ich die
Spalte zwischen den Spiegeln (sie Elektronenmikroskopbild).  Das
Fourierspektrum $\widetilde T_\textrm{line}$ dieser Zeile:
\begin{align}
  \widetilde T_\textrm{line}(k_x) & = \sum_{p=-N/2}^{N/2+1} T_p(k_x) \\
  &= \sum_{p=-N/2}^{N/2+1} e^{ik_x w p} \left(w
...
\begin{align}
  |\widetilde T_\textrm{line}(0)|^2 = \sinc^2(k_p)
\end{align}
