...
The lineage tree of two developing \celegans\ embryos is the \cm{reproducible development}
...
threshold dose approximately 50 cells develop during the two
hours.

For a dose above the threshold the development is slowed due to
phototoxicity and the number of cells after the experiment reduce.
...
For the orange data points, the integration time $\tau$ for each slice
is \unit[100]{ms} and for the green data points the integration time
...
time and less intensity are less phototoxic.

An earlier study with tobacco plants \cite{Dixit2003} obtained similar
results: There is a threshold dose below which no phototoxicity ca be
detected, and the threshold decreases with light intensity. In the
plants Dixit and Cyr show that the damage is caused by reactive oxygen
species and explain the shift of the phototoxicity threshold by the
limited capacity of the cells scavenging system for those
radicals. They also predict the existence of redox-sensitive
checkpoints in the mitotic division cycle.
...
A transition between a ground state $S_0$ to an excited state $S_1$     \cma{electronic transition}
...
Photons with wavelengths below \unit[200]{nm} have sufficient energy     \cma{wavelength range of fluorophores}
...
\begin{figure}[!hbt]
  ...
  \label{fig:flu-level}
\end{figure}
The Jablonski diagram in \figref{fig:flu-level} summarizes information    \cma{energy levels}
...
As described above, electronic excitation in general also leads to the   \cma{solution rapidly damps vibrations}
...
During emission of a fluorescence photon a Franck-Condon state of the  \cma{energy-loss = color change}
...
There is a small probability that the HOMO electron doesn't relax by   \cma{spin flip forbidden}
