...
can be measured in I${}^5$M but there is still background signal
...
\label{sec:etendue}
The etendue $\mathcal{E}=\frac{\pi}{4}(D_\textrm{field}\,\textrm{NA})^2$ --- also called
...
\nomenclature{$\mathcal{E}$}{Etendue, information capacity, light
  gathering capacity or space-bandwidth product; its value is related
  to the number of point spread functions that can be resolved in the
  field.}
...
diffraction-limited lenses image formation is a linear in intensity
and three-dimensionally shift-invariant. 

In this analysis, we will draw attention to an important limitation of
the wide-field microscope : The restricted to only a half-space
aperture leads to the Missing Cone Problem --- an empty area in the
optical transfer function , which will pay for any information. The
picture makes itself felt in that fluorescent light from out-of -
focus molecules not through destructive interference disappears but
contributes to a blurred background.

Bei dieser Analyse wird man auf eine wichtige Limitierung des
Weitfeld-Mikroskops aufmerksam: Der auf nur einen Halbraum
eingeschraenkte Aperturwinkel fuehrt unweigerlich zum Missing Cone
Problem bei dem es zu einem leeren Bereich in der optischen
Transferfunktion kommt, wo keine Informationen uebertragen werden. Im
Bild macht sich das dadurch bemerkbar, dass Fluoreszenslicht von
out-of-fokus zu einem verschwommenen Hintergrund beitraegt.
