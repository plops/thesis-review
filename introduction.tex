...
the carriers into a charge amplifier. \cma{charge amplifier}
 This consists of a much smaller   
...
line is one, according to equation \eqref{eq:variance}.
...
the dark images reflect the readout noise. Using the calibration gain,
the read noise can be specified as photoelectrons$/$pixel
(\unit[]{e/px}).
...
\ref{sec:python-readnoise-eval} I list an alternative Python
implementation of this algorithm.

The major source for read noise is the charge amplifier. Its noise is
added uniformly to every image pixel. For readout frequencies above
\unit[1]{MHz} the read noise increases with the square root of the
...
developed --- the electron-multiplying CCD \citep{Mackay}.
...
generate more charge carriers via a process called impact
...
$M=(1+p)^{536}\approx 1015$ is so high, that even read noise at \unit[17]{MHz}
readout speed can be neglected.

This amplification process consumes a lot of energy and requires
elaborate cooling of the detector chip as the gain is temperature
dependent.

Unfortunately, the statistical nature of impact ionization leads to a
new noise source. Since the noise corresponds to an uncertainty in
gain, the noise has a multiplicative effect on the
signal. \cite{Robbins2003} analyzed the amplification process and
shows that the multiplicative noise has the effect of halving the
apparent quantum efficiency of the detector.
