...
\begin{summary}
  ...
  in the widefield microscope and I describe its principle limitations
...
biological function.  Besides localizing molecules it permits
measurement of physical quantities inside of the sample. Fluorescent labels
...
accessible as in any conventional widefield microscope.
...
addition, analyzing the shape of the host cell nuclei can help to
...
\citep{Stiernagle2006}, safe in use and allows the comparison of
phototoxicity between different microscopes \citep{Tinevez2012}. The
basis of this system are embryos of the small invertebrate
\celegans. The adult form is approximately \unit[1]{mm} long.  Their
anatomy and development are comparatively simple and have been well
characterized \citep{Sulston1977,Durbin1987}.
...
$\unit[21\pm 1]{\si{\degreeCelsius}}$, two different embryos will develop
...
widefield microscope. From a very low excitation up to a certain
...
\nomenclature{$\Omega$}{Excitation dose in $\si{\joule/(\centi\meter^2\textrm{stack})}$} 
...
\begin{summary}
  The widefield fluorescence microscope does not excite fluorophores
...
representation of the detection path of a widefield microscope.
...
\end{align}
with the object-sided collection half-angle $\alpha$ and refractive
index $n$, the corresponding parameters $\alpha'$ and $n'$ on the
image side and the magnification $\beta$.
...
lens with a non-linear distortion in the back focal plane, such as given by the concept of a thin lens used at large angles, will fail to
...
invariant for small regions of the field plane and allows expression
of the image formation with linear systems theory.
...
\label{sec:wave-image-formation}
In the following section I will describe how the image on the camera
...
\end{align}
with the coordinate $\r$, vacuum wavelength $\lambda_0$, refractive
index $n$ and wavelength $\lambda$ in the immersion medium.
...
the fluorophores:
\begin{align} \label{eq:image-convolution}
...
origin.  This means that in a conventional widefield microscope it is
...
Structured \cma{optical sectioning with confocal
  microscope} \label{sec:sectioning-intro} illumination can be used to
remove this out-of-focus light.  Two major incarnations of this
technique are in current use.  On the one hand there is the confocal
microscope\label{sec:confocal} in which mechanically scans a single
point spread function over the focal plane. This approach allows
blocking out-of-focus light optically. The concept was invented by
Minsky in the late fifties \citep{Minsky1961}, the first confocal
microscope with a laser was built in the early seventies
\citep{davidovits1971scanning} and was commercialized after
...
\subsection{Illumination in a widefield epifluorescence microscope}
...
formation in a widefield microscope. For well-corrected,
...
Nowadays, all widefield microscopes use silicon-based cameras to
...
their measurements to effective photoelectrons.  This allows the
comparison of images of different microscope systems (confocal,
spinning disk or widefield) or and facilitates the use of
...
\begin{figure}
  ...
  \caption{{\bf left:} Image of a defocused area on a fluorescent
    plane sample. This is used for calibrating the detector. {\bf
      right:} Result of a detector calibration. Using the slope of the
    curve the arbitrary analog-to-digital units are converted into
    effective photoelectrons. The point of interception at the
    $y-$axis gives the detector read noise.}
...
\end{align}
This equation permits comparison between all the cameras that I can
...
sensor the global shutter can  start the exposure in all pixels simultaneously,
...
once all pixels have started capturing light. This allows using the
...
The method I presented for CCD calibration in section \ref{sec:photon-noise} can be applied to measure
...
distribution (from exactly the same sample and illumination) with and without
...
allows comparison between images that were created with different
microscopes.
