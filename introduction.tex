...
\end{figure}
Assuming that the excited fluorophores in the sample give rise to a \cma{Ewald sphere}
...
$\nu_x'=n'\sin\alpha'$ with $n'=1$ in image space. According to the
Abbe sine condition (\ref{eq:sine-condition}) the transversal spatial
...
\end{align}
Note that I use the transversal magnification to scale the arguments
of the functions, so that the result is given in image space spatial
frequencies\footnote{Unfortunately my notation is slightly problematic
  here. I assume that $z-$sampling occurs by stepping the sample
  through the object space while the camera is fixed in the focal
  plane of the tube lens. That way there is no difference between
  axial coordinates $r_z$ and $r'_z$ in object and image.}.
...
Ein Detektor kann nur die Intensitaet messen, die nicht linear von der
Amplitude abhaengt. Interessanterweise kann die Bildentstehung dennoch
vollstaendig auf eine lineare Operation zurueckgefuehrt werden:

A focal plane detector can only measure the intensity $I'$ which
depends non-linear on the amplitude of the field $u'$. Interestingly,
the image formation can still be ascribed to a linear operation:
\begin{align}
...
Das beschreibt, wie gut Frequenzen ubertragen werden.
...
It is noteworthy that the axial region above and below the origin is
always empty for $\alpha<\pi/2$, i.e.\ for all practical objectives. 

This means that in a conventional wide-field microscope it is
impossible to focus a (defect-free) fluorescent plane sample because
low spatial frequencies do not attenuate with defocus. This effect is
also called ``missing cone problem'' \citep{Streibl1984}.


Dieser Effekt wird auch Missing Cone problem genannt.
