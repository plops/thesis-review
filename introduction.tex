...
For the orange data points, the integration time for each slice is
\unit[100]{ms} and for the green data points the integration time is
five times higher.
...
\end{align}
with integration time $\tau$ and radiant flux $\Phi_e$ of the
excitation light, as measured in the pupil and area $A$ of the
illuminated field and the number of slices $n=41$.

Naively one would assume that it shouldn't make any difference if the
excitation light is administered in \unit[100]{ms} or \unit[500]{ms}
but the data show that a longer exposure time and less intensity are
less phototoxic.

A study with tobacco plants \citep{Dixit2003} obtained similar
results: There is a threshold dose below which there is no measurable
phototoxicity and the threshold decreases with light intensity. In the
