...
a sphere with radius $n f$ and called \emph{aplanatic sphere}. FIXME
translate: Die Brechzahl $n$ steht dabei fuer ein etwaiges
objektseitiges Immersionsmedium. I depict this surface as two circle
segments with a bold red curve on each of the lenses in
...
FIXME translate: Die \figref{fig:widefield-microscope} zeigt wie
mittels Laser das Bildfeld in einem widefield fluorescence microscope
ausgeleuchtet wird.
...
defined by the acceptance angle $\alpha$ of the objective.
...
depends on the charge $Q$ $U=Q/C$. The designs try to minimize the
capacitance $C$ of the read node to maximize sensitivity
\citep{Pawley2006}. {\color{red} FIXME check equation}
...
Poissonian probability mass function (denoted as '$\pois$'):  % FIXME rainer will density
\begin{align}
...
mode as indicated by the blue-green line in the top left.  FIXME translate:
Aber die impact ionization die in den Verstaerkerregistern with
advantage genutzt wird um tritt mit geringerer Wahrscheinlichkeit auch
bei kleinen Spannungen auf und fuegt dem Signal so um die
$\unit[200]{e^-/frame}$ an zusaetzlichen Rauschen hinzu. Dieses wird
clock-induced charge genannt.
