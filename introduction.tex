...
depends non-linearly on the amplitude of the field
$u'$. Interestingly, the image formation can still be ascribed to a
linear operation:
\begin{align}
...
\end{align}
This is just a single application of the convolution theorem. Note
that the complex conjugation of the second factor $h^*(\r')$ results
in an inversion of the argument of $\widetilde h^*(-\vnu')$.
...
It is noteworthy that a cone shaped region around the axis above and
below the origin is always empty for aperture angles $\alpha<\pi/2$
that collect only light from one half space, i.e.\ for all practical
objectives.

This means that in a conventional wide-field microscope it is
impossible to bring into focus a (defect-free) fluorescent plane
because low spatial frequencies do not attenuate with defocus. This
effect is also called ``missing cone problem'' \citep{Streibl1984}.
...
In diesem Abschnitt habe ich ein theoretisches Modell eingefuehrt,
dass es erlaubt die Bildformung im Weitfeld-Mikroskop zu beschreiben.
Fuer gut korrigierte diffraktionslimitierte Objektive ist diese
Operation linear in der Intensitaet und 3D translationsinvariant.

Verglichen mit anderen Methoden der Mikroskopie, z.B. differentiellem
Interferenzkontrast, ist es daher sehr einfach, aus einer gegebenen 3D
Fluorophorverteilung auf die Bilder zu schliessen, die ein
Fluoreszensmikroskop davon liefert. Insbesondere kann mit dem Traeger
der optischen Transferfunktion Vorraussagen ueber die bestmoegliche
transversale und axiale Aufloesung machen.

Bei dieser Analyse wird man auf eine wichtige Limitierung des
Weitfeld-Mikroskops aufmerksam: Der auf nur einen Halbraum
eingeschraenkte Aperturwinkel fuehrt zum Missing Cone Problem ---
einem leeren Bereich in der optischen Transferfunktion, wo keine
Informationen uebertragen werden. Im Bild macht sich das dadurch
bemerkbar, dass Fluoreszenslicht von out-of-fokus Molekuelen nicht
durch destruktive Interferenz verschwindet sondern zu einem
verschwommenen Hintergrund beitraegt.

Im 4Pi-Mikroskop wird das Licht zweier entgegengesetzer Objektive zur
Interferenz gebracht. Die optische Transferfunktion hat deshalb keinen
Missing Cone und Bilder auf der Kamera enthalten nur Information aus
der Fokusebene. Leider stellt dies unheimlich grosse Anforderungen an
die Qualitaet der Linsen und funktioniert auch nur fuer wenige
Mikrometer duenne, transparente Spezimen.

Ein anderer Ansatz, dass missing cone problem zu beheben, ist
strukturierte beleuchtung.  dann werden jedoch einige bilder benoetigt
oder gescannt (confocal) ausser bei picosim wo geschickte verwendung
von polarization
