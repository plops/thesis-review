...
from the patent \cite{Mathae2003}  %US6504653 (FIXME) 
--- many spherical surfaces and
various glasses must be combined to obtain a device that is
apochromatic, aplanatic and isoplanatic \citep{Gross2005}.
...
It corresponds to lens with corrected spherical aberrations, linear
coma (sagittal and tangential) near the axis and fulfills the Abbe
Sine condition. This correction ensures that the focal length --- a
parameter that is normally only defined for paraxial rays --- is
identical for all collected rays. Such a lens carries out a Fourier
transform from the front to the back focal plane with linear scaling,
even for high angles:
\begin{align}
  \beta = \frac{|\vnu_t|}{|\vnu'_t|}=\frac{n \sin\alpha}{n' \sin\alpha'}
\end{align}
 Only then, the image is similar to the object.

 The longitudinal magnification $\zeta=\Delta\nu_z/\Delta\nu'_z$, with
 $\Delta\nu_z=n/\lambda - \nu_z$ giving the distance from a point on
 the Ewald sphere to the point on the Ewald sphere that intersects
 the $\nu_z-$ axis, depends on the transverse frequency $|\vnu_t|$:
 \begin{align}
   \zeta = \frac{n/\lambda - \nu_z}{n'/\lambda - \nu'_z}=\frac{n/\lambda - \sqrt{(n/\lambda)^2-|\vnu_t|^2}}{n'/\lambda - \sqrt{(n'/\lambda)^2-|\vnu_t|^2/\beta^2}}. %=\frac{n(1-cos\alpha)}{n'(1-cos\alpha')}=\frac{n\sin^2(\alpha/2)}{n'\sin^2(\alpha'/2)} %Herschel condition
 \end{align}
...
% Staeble-Lihotzky condition for isoplanatism is fulfilled but it is
% the same as the Abbe Sine condition if spherical aberration is
% corrected. Prof Gross IAT 10
...
strokes on the lenses in \figref{fig:widefield-microscope}~b).
...
quantitative expressions I limit myself to the scalar problem.
...
Ewald sphere.  \nomenclature{Ewald sphere}{Transfer function of free
  space} Rays that fall into the acceptance angle $\alpha$ of the
objective fill out a segment of the Ewald sphere with the
three-dimensional frequency spectrum $\widetilde u(\vnu)$ of the
sample as shown in \figref{fig:widefield-microscope}~a). Scaling this
