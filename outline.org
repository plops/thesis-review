...
- Jetzt stellen wir dar, wie man mit dem spatio-angularen Mikroskop
  ...
  Untersuchungen und Methodenentwicklung in Betracht.
...
- Strukturierte Beleuchtung ist eine hilfreiche Methode, um das
  ...
  Wissen ueber die genaue Fluorphorverteilung jedoch sehr wichtig und
...
- Wie wir bereits zu Beginn des Kapitels erlaeutert haben, genuegt uns
  ...
  Samples. 
- Laut Protokoll wollen wir die Fluorophorverteilung in 20
  unterschiedlichen Z-Positionen ermitteln. Uns steht eine Prognose
  der dreidimensionalen Fluorophorverteilung zur Verfuegung. Im
  Computer schneiden wir die Kugelverteilung der Prognose mit der
  aktuell beobachteten Ebene (Einstellung des motorisierten
  z-stages). Das gibt uns einige Kreise, die angeben wo sich in-fokus
  Nuklei befinden. Fuer jeden dieser Kreise stellen wir an der
  entsprechenden Position auf dem focal plane SLM eine Scheibe (mit
  mindestens 4 um Durchmesser, um Winkelkontrolle nicht zu
  untergraben) dar.
