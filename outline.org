...
* the concept of spatio-angular microscopy
  - hier zeigen wir, wie unser spatio-angulares mikroskop funktioniert
  - zunaechst motivieren wir diese beleuchtungsart anhand zweier
    generischen, in gewoehnlichen samples oft vorkommenden beispielen.
  - dann beschreiben wir entscheidungen bezueglich der anordnung der
    ...
    treffen mussten. von allen bisher veroeffentlichten ansaetzen der
    beleuchtungssteuerung in der mikroskopie kommt unserem ansatz das
    lichtfeldmikroskop (levoy) am naechsten. wir erklaeren die
    unterschiede zwischen beiden techniken und argumentieren, warum
    unser ansatz dem lichtfeldmikroskop nicht nachsteht und in der
    praxis auch einige vorzuege bietet.
  - es stellt sich heraus, das eine effektive (FIXME effiziente?), die
    phototoxizitaet mindernde, nutzung des spatio-angularen mikroskops
    recht viel wissen ueber die probe bedarf (fluorophor- und
    brechzahlverteilung). die auswahl passender beleuchtungsmuster
    erfordert eine vorhersage, oder zumindest das verstaendnis, der
    dreidimensionalen lichtverteilung im objektraum. im letzten
    abschnitt dieses kapitels beruehren wir themen der
    bildverarbeitung und ziehen auch die parallele zur medizinischen
    tumortherapie.
