...
** optical components
 ...
 - fig:memi-real zeigt schematisch den entsprechend angepassten
   ...
   (einige der folgenden details aus deliverables entnommen, die vom
   projekt consortium als confidential eingestuft wurden.)
...
*** The illumination system
 ...
 - Quantitative Experimente erfordern eine homogene Ausleuchtung von
   sowohl pupil plane als auch focal plane SLM.
 ...
 - Die von uns verwendete LED \footnote{gekauft von Reichelt
   ...
   Flaeche messen} hat eine zu grosse leuchtende Flaeche. Das heisst,
   wir verlieren zwar Licht, andererseits ist es Leicht eine homogene
   Ausleuchtung zu erreichen. Ausserdem kann die LED schnell
   elektronisch ein- und ausgeschalten werden\footnote{Der DPSS Laser
   kann nicht schnell elektronisch geschalten werden, sondern wird mit
   einem Akusto-optischem Modulator geschaltet (FIXME siehe spaetere
   ref section).}.

 - Die LED ist jedoch 
 
 - Wenn wir einen Laser benutzen, dann senden wir den parallelen
   Strahl zunaechst in ein Buendel (FIXME Loptec 1.1mm Durchmesser, 2m
   Laenge, beam broadening 3.18 grad, ref D8_4) statistisch verteilter
   Fasern um die gaussfoermige Intensitaetsverteilung zu
   randomisieren.  (FIXME ich vermute mal, dass das faserbuendel nicht
   mehr quadratisch auf einer seite ist, ist das richtig?)
   
 - Ein rotierendes Mikrolinsenarray (.5x.5 mm grosse Linsen, 23.5mm
   focal length (FIXME ist das auch im letzten prototypen noch
   korrekt, eine der zwischenergebnisse war, die fokuslaenge der
   mikrolinsen zu verkuerzen, um mikrochipping im tunnel zu
   kompensieren , \pm0.23 grad beam broadening) mischt das Licht, so
   dass waehrend einer Belichtungszeit der Kamera moeglichst viele
   Modenprofile das Spezimen beleuchten.  Durch diese Vorgehensweise
   koennen wir das Laserlicht, das zunaechst hohe oertliche
   Kohaerenz---also eine geringe Etendue---aufweist, an die Etendue
   der nachfolgenden Optik (durch das MMA begrenzt) anpassen.

 - Durch mehrfache Reflexion in einem Lichttunnel (quadratische
   2.5mmx2.5mm cross section, 250mm laenge) \footnote{Der Tunnel (rod
   integration system, light pipe, D8_2_v2 is a good document) hat
   einen quadratischen Querschnitt. priv. comm. mit Prof. Herbert
   ...
   erzeugen wir eine homogene Lichtverteilung in F'''.  Eine
   Relais-Optik vergroessert den Tunnelausgang des Tunnels auf 4x4mm^2
   in die Ebene F'''.
 


 - erfahrungen beim bau der beleuchtungshomogenisierung
   - buendelende sollte den integrator rod eintritt deutlich
     ueberdecken
   - mikrolinsen homogenizer sollte eher kuerzere brennweite haben, um
     mikrochipping an den ecken der zementierten glasspiegel zu
     kompensieren
   - faserbuendel muss zentriert und im winkel justiert werden
 - schlierenoptik 
   - zwei aufgaben:
     - 1:1 bild von feld maske in feld stop
     - abbildung von mma nach unendlich
   - fokus laenge so gewaehlt, dass 700nm licht nicht durch feldstop
     kommt (TODO FIXME vignitiert die linse oder ihre mechanik?)
   - hat lateralen fehler (TODO was wuerde passieren, wenn ich die
     lampe wechsle?)
 - double telecentric relay with magnification 2
 - tubelens telescope braucht 3 bewegliche gruppen, um stationaeres
   bild des mma zu gewaehrleisten
 - moxtek soll in tranmission genutzt werden
   - das macht das system jedoch asymmetrisch und fuehrt zu
     hauptsaechlich astigmatismus und lateral color (D8.9)
   - 0.7mm duenne platte
 ...
 - druecke winkel in seconds of arc aus? (FIXME einheit nachschauen,
   lieber nicht, das sind grad/3600, die si einheit ebener winkel ist
   rad, fuer raumwinkel sr, 1995: namely the radian and the steradian,
   as dimensionless derived units, the names and symbols of which may,
   but need not, be used in expressions for other SI derived units, as
   is convenient)
