...
positioned exactly on the detection pinhole. Conversely, the
out-of-focus bead (2) gives rise to a blurred image. Its fluorescence
light is distributed over a large area.
...
microscope. \figref{fig:hourglass-all}~c) shows a configuration
of the specimen with two in-focus beads (5) and (6), and one
out-of-focus bead (7).  If both (5) and (6) should be illuminated
simultaneously, i.e.\ the entire field is to be exposed by an extended
light source, there is no way to avoid exposure of the out-of-focus
bead (7) with just angular control of the illumination.
...
etendue (also called space-bandwidth product (FIXME check
definitions)) of the light source must be large enough, so that both
the pupil P as well as the field F are fully illuminated.
